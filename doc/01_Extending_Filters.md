...
- add the condition(s) to the opendxp listing which will load all objects the index search found

For convenience reasons you can extend the ``OpenDxp\Bundle\AdvancedObjectSearchBundle\Event\AbstractFilterListener`` and register it
...
- ``public function onListing(FilterListingEvent $event)`` to add the condition(s) to the opendxp listing
...
``advanced_object_search.index_filter`` event (``OpenDxp\Bundle\AdvancedObjectSearchBundle\Event::INDEX_FILTER``). As mentioned
...
will be loaded using a basic opendxp listing of that class which enables you to add more mysql queries to that listing.

For that you can use the ``advanced_object_search.listing_filter`` event (``OpenDxp\Bundle\AdvancedObjectSearchBundle\Event::LISTING_FILER``).
...
For that you have to create a javascript class that extends ``opendxp.bundle.advancedObjectSearch.searchConfig.ResultExtension`` 
...
```js
opendxp.registerNS("opendxp.bundle.AppBundle.AdvancedObjectSearch.Extension.*");
opendxp.bundle.AppBundle.AdvancedObjectSearch.Extension.ArticleNumber = Class.create(opendxp.bundle.advancedObjectSearch.searchConfig.ResultExtension, {
...
of the opendxp plugin of your bundle.

```js
opendxp.registerNS("opendxp.bundle.AppBundle.*");

opendxp.bundle.AppBundle.Bundle = Class.create({
    getClassName: function() {
        return "opendxp.bundle.AppBundle.Bundle";
    },

    initialize: function() {
        document.addEventListener(opendxp.events.onAdvancedObjectSearchResult, this.onAdvancedObjectSearchResult.bind(this));
    },
    ...
    onAdvancedObjectSearchResult: function (e) {
        ...
        extensionBag.addExtension(new opendxp.bundle.AppBundle.AdvancedObjectSearch.Extension.ArticleNumber());
    }
});

var AppBundle = new opendxp.bundle.AppBundle.Bundle();
```
