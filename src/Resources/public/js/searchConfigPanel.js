...
opendxp.registerNS("opendxp.bundle.advancedObjectSearch.searchConfigPanel");
opendxp.bundle.advancedObjectSearch.searchConfigPanel = Class.create(opendxp.element.abstract, {
    ...
    initialize: function(data) {
        ...
        this.tab = new Ext.TabPanel({
            activeTab: 0,
            id: this.getTabId(),
            iconCls: "opendxp_bundle_advancedObjectSearch",
            closable: true,
            forceLayout: true,
        ...
        this.tab.on("destroy", function () {
            opendxp.globalmanager.remove(this.getTabId());
        }.bind(this));
    ...
    getTabId: function() {
        if(!this.tabId) {
            if(this.data && this.data.id) {
                this.tabId = "opendxp_search_" + this.data.id;
            } else {
                this.tabId = "opendxp_search_" + uniqid();
            }
        }
        return this.tabId;
    },
    ...
    getConditions: function() {
                ...
                listeners: {
                    change: function( item, newValue, oldValue, eOpts ) {
                        ...
                        if(newValue != oldValue) {
                            ...
                            this.conditionPanel = new opendxp.bundle.advancedObjectSearch.searchConfig.conditionPanel(newValue);
        ...
        if(this.data.classId) {
            this.conditionPanel = new opendxp.bundle.advancedObjectSearch.searchConfig.conditionPanel(this.data.classId, this.data.conditions);
            this.conditionPanelContainer.add(this.conditionPanel.getConditionPanel());
        }
        ...
        return new Ext.Panel({
            ...
            iconCls: "opendxp_bundle_advancedObjectSearch_filter",
    ...
    getResults: function() {
        this.resultPanel = new opendxp.bundle.advancedObjectSearch.searchConfig.resultPanel(this.getSaveData.bind(this), this.data.gridConfig);
    ...
    getSaveAndShare: function () {
        ...
        this.settingsForm = Ext.create('Ext.form.FormPanel', {
            ...
            iconCls: "opendxp_bundle_advancedObjectSearch_saveAndShare",
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                if (rdata && rdata.success) {
                    opendxp.helpers.showNotification(t("success"), t("bundle_advancedObjectSearch_save_success"), "success");
                    ...
                    this.tabId = null;
                    opendxp.globalmanager.add(this.getTabId(), this);
                    ...
                    opendxp.helpers.showNotification(t("error"), t("bundle_advancedObjectSearch_save_error"), "error",t(rdata.message));
                }
            }.bind(this)
        });
    ...
    delete: function() {
        if(!this.data && !this.data.id) {
            opendxp.helpers.showNotification(t("error"), t("bundle_advancedObjectSearch_delete_error"), "error");
        } else {
            Ext.MessageBox.show({
                title:t('delete'),
    ...
    toggleShortCut: function() {
        if(!this.data && !this.data.id) {
            opendxp.helpers.showNotification(t("error"), t("bundle_advancedObjectSearch_short_cut_error"), "error");
        } else {
            Ext.Ajax.request({
