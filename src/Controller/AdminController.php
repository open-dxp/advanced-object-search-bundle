...
class AdminController extends UserAwareController
{
    ...
    public function gridProxyAction(Request $request, Service $service, EventDispatcherInterface $eventDispatcher): JsonResponse | Response
    {
        ...
        if ($request->request->has('data')) {
            return $this->forward('OpenDxp\Bundle\AdminBundle\Controller\Admin\DataObject\DataObjectController::gridProxyAction', [], $request->query->all());
        } else {
            ...
            if (!$this->getOpenDxpUser()->isAdmin()) {
                $userIds = $this->getOpenDxpUser()->getRoles();
                $userIds[] = $this->getOpenDxpUser()->getId();
            ...
            if (!empty($ids)) {
                ...
                $list->setOrderKey(' FIELD(id, ' . implode(',', $ids) . ')', false);
            } else {
    ...
    public function saveAction(Request $request): JsonResponse
    {
        ...
        if ($id !== null) {
            ...
            $savedSearch->setOwner($this->getOpenDxpUser());
        }
        ...
        $savedSearch->setShareGlobally($this->getOpenDxpUser()->isAdmin() && $data->settings->share_globally);

        $config = ['classId' => $data->classId, 'gridConfig' => $data->gridConfig, 'conditions' => $data->conditions];
        $savedSearch->setConfig(json_encode($config, JSON_THROW_ON_ERROR));
    ...
    public function loadSearchAction(Request $request): JsonResponse
    {
        ...
        $savedSearch = SavedSearch::getById($id);
        if ($savedSearch) {
            $config = json_decode($savedSearch->getConfig(), true, flags: JSON_THROW_ON_ERROR);
                ...
                foreach ($config['gridConfig']['columns'] as &$column) {
                    if (!($column['isOperator'] ?? false)) {
                        ...
                        if ($fieldDefinition) {
                            ...
                            $column['layout'] = json_decode(json_encode($fieldDefinition, JSON_THROW_ON_ERROR), flags: JSON_THROW_ON_ERROR);
                            if ($width) {
                    ...
                    $helperColumns[$column['key']] = json_decode(json_encode($column, JSON_THROW_ON_ERROR), flags: JSON_THROW_ON_ERROR);
                }
